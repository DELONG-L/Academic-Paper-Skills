...
主流期刊对投稿图表的要求汇总。表中的字号是最终放置后的有效
字号。House soft default 建议在 3x 源画布使用约 24pt 或更大的显式字号；
再缩放到栏宽并用 hard final-width gate 检查最终渲染。**画图前先查目标期刊**，把栏宽、
字号、DPI、推荐字体、矢量格式偏好记下来再开 matplotlib。
...
**坑**：Nature 极其强调最终尺寸合规。使用 3x 源画布后，必须在最终
栏宽渲染并按 mm 与有效字号复核，不能把源码 24pt 当作最终合规证明。
