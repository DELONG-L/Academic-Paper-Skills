...
每一节给一份**可直接运行的 Python 代码**——直接复制改数据就能出图。所有代码假设已经调过 `setup_style()`。尺寸与字号是可修改的示例；替换为目标稿件的实际尺寸，并在最终宽度检查。示例数据均为演示数据。
...
for g, c in zip(['A', 'B'], [OKABE[2], OKABE[6]]):
    ...
    ax.text(0.05 if g=='A' else 0.05, 0.95 if g=='A' else 0.88,
            f'{g}: r={r:.2f}, p={p:.1e}',
            transform=ax.transAxes, fontsize=8.0, color=c, va='top')
...
def sig_bracket(ax, x1, x2, y, h, text, fontsize=8.0):
    """在 (x1, x2) 之间画一道带文字的桥。"""
...
hm.collections[0].colorbar.ax.tick_params(labelsize=8.0)
...
ax.tick_params(labelsize=8.0)
ax.set_title('Feature correlations', fontsize=8.0)
...
for ax, label in zip(axes.flat, ['a', 'b', 'c', 'd']):
    ax.text(-0.20, 1.05, label, transform=ax.transAxes,
            fontsize=8.0, fontweight='bold', va='top', ha='right')
