...
def export_figure(
    ...
    for fmt in formats:
        if fmt in {"jpg", "jpeg"}:
            print(f"[paper-figures-tables] WARNING: skipping {fmt} — "
        ...
        saved.append(path)
        print(f"[paper-figures-tables] wrote {path}")
...
def _grayscale_from(fig, basename: str, dpi: int) -> str | None:
    ...
    try:
        ...
        print("[paper-figures-tables] Pillow not available; "
    ...
    print(f"[paper-figures-tables] wrote {gray_path} (grayscale preview)")
    return gray_path
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="paper-figures-tables figure exporter")
    p.add_argument("cmd", choices=["demo"], help="`demo`: 跑一张演示图导出 4 种格式")
    p.add_argument("--out", default="./figure_demo",
                   help="输出 basename (默认 ./figure_demo)")
    args = p.parse_args()
    if args.cmd == "demo":
        _demo(args.out)
    return 0
